...
/*			if (RTL_R8(PHYstatus) & _100bps)
			{
				// IGPIO21 pin mux : 0x9800731C [1:0] =00
				writel(readl(tp->mmio_clkaddr + 0x31C) & ~0x00000003, (tp->mmio_clkaddr + 0x31C));
				// IGPIO32 : out direction => 0x98007100[21]=1, out data => 0x98007104[21]
				writel(readl(tp->mmio_clkaddr + 0x100) | 0x00200000, (tp->mmio_clkaddr + 0x100));
			...
			else
			{
				// IGPIO21 pin mux : 0x9800731C [1:0] =00
				writel(readl(tp->mmio_clkaddr + 0x31C) & ~0x00000003, (tp->mmio_clkaddr + 0x31C));
				// IGPIO21 : out direction => 0x98007110[21]=1, out data => 0x98007104[21]
				writel(readl(tp->mmio_clkaddr + 0x100) | 0x00200000, (tp->mmio_clkaddr + 0x100));
				writel(readl(tp->mmio_clkaddr + 0x104) & ~0x00200000, (tp->mmio_clkaddr + 0x104));
			}
*/
...
/*			// IGPIO21 pin mux : 0x9800731C [1:0] =00
			writel(readl(tp->mmio_clkaddr + 0x31C) & ~0x00000003, (tp->mmio_clkaddr + 0x31C));
			// IGPIO21 : out direction => 0x98007100[21]=1, out data => 0x98007104[21]
			writel(readl(tp->mmio_clkaddr + 0x100) | 0x00200000, (tp->mmio_clkaddr + 0x100));
			writel(readl(tp->mmio_clkaddr + 0x104) & ~0x00200000, (tp->mmio_clkaddr + 0x104));
