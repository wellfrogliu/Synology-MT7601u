...
static char *snd_pcm_id[] = {SND_REALTEK_DRIVER}; // multiple PCM instances in 1 sound card
...
static void snd_card_capture_lpcm_timer_function(unsigned long data)
{
    ...
    {
        ALSA_WARNING("[hw_ptr %d appl_ptr %d %d @ %s %d]\n", (int)runtime->status->hw_ptr, (int)runtime->control->appl_ptr, (int)runtime->buffer_size, __FUNCTION__, __LINE__);
    }
    ...
    if(nRingDataFrame >= runtime->period_size)
    {
        ...
        // The size need to copy CAN'T exceed the size of destination buffer
        if((nPeriodCount * runtime->period_size) > runtime->buffer_size)
        {
            nPeriodCount = runtime->buffer_size / runtime->period_size;
/*            ALSA_VitalPrint("alsa cap copy full.. %x %x\n", nPeriodCount
